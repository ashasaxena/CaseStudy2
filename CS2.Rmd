...
f<- count(PD.renamed$Country) #Generates frequency table.  Number of Observations(breweries)/State
f
'%nin%' <- Negate('%in%')
pu <- unique(PD.renamed$Country)
hu <- unique(CountryHDI$Country)
a<- pu[pu %nin% hu]  #Finds unique Countries in the procrastination data set that are not in the HDI dataset.  
a
